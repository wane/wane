...
export class WaneTemplateMarkupLexer extends Lexer<MarkupToken> {
  ...
  private plainAttributeNameState: State = () => {
    ...
    switch (true) {
      case isSpaceCharacter(c):
        this.getToken(TagToken).getCurrentAttribute(PlainAttribute).declareBoolean()
      // fallthrough
      case is(Unicode.Solidus)(c):
      case is(Unicode.GreaterThanSign)(c):
      case isEof(c):
      ...
      case is(Unicode.RightCurlyBracket)(c):
        this.parseError()
        this.getToken(TagToken).getCurrentAttribute(PlainAttribute).appendToName(c as string)
        break
      default:
        this.getToken(TagToken).getCurrentAttribute(PlainAttribute).appendToName(c as string)
    }
  ...
  private plainAttributeValueState: State = () => {
    ...
    switch (true) {
      ...
      case isEof(c):
        ...
        break
      default:
        this.getToken(TagToken).getCurrentAttribute(PlainAttribute).appendToValue(c as string)
    }
  }
  ...
  private expressionOrInvocationAttributeNameState: State = () => {
    ...
    switch (true) {
      case isAttributeNameCharacter(c):
        this.getToken(TagToken).getCurrentAttribute(NonPlainAttribute).appendToName(c as string)
        break
      case is(Unicode.RightSquareBracket)(c):
        if (!this.getToken(TagToken).isCurrentAttributeExpressionAttribute()) {
      ...
      case is(Unicode.RightParenthesis)(c):
        if (!this.getToken(TagToken).isCurrentAttributeInvocationAttribute()) {
          this.parseError() // unmatched wrappers for attribute name
        }
        this.switchTo(this.afterExpressionOrInvocationAttributeNameState)
        break
  ...
  private expressionOrInvocationAttributeValueState: State = () => {
    ...
    switch (true) {
      ...
      case isEof(c):
        ...
        break
      default:
        this.getToken(TagToken).getCurrentAttribute(NonPlainAttribute).appendToValue(c as string)
    }
  }
